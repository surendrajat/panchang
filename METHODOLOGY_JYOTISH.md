...
anchored to **23.8635° at J2000**.

> **A precision footnote that matters for rigor.** "Lahiri" has slightly
> different numerical realizations. Ours follows Drik Panchang's *computational*
> value (23.8635° at J2000) so the **panchanga's** nakshatra/tithi end times match
> Drik. Swiss Ephemeris's `SE_SIDM_LAHIRI` (tagged "official Indian government")
> is **0.39′ lower** (23.857° at J2000). That 0.39′ ≈ 1.6 seconds of birth time —
> far below any practical threshold — but it is the bulk of the tiny gap between
> our grahas and Swiss Ephemeris (§8).
...
standard. (The grahas are unaffected — they match Drik too, to ≤0.04′.)
...
- **Grahas vs Swiss Ephemeris**, 1925–2040 (`tests/regression/kundli-vs-swisseph.test.ts`):
  all nine grahas match to **≤ 0.75′**. Of that, a constant ~0.39′ is the Lahiri
  ayanamsa variant (§3.3); the rest (~0.3′) is theory difference. Reference values
  produced by running **pyswisseph** directly.
- **Grahas vs Drik Panchang** (`tests/regression/kundli-vs-drik.test.ts`): match
  to **≤ 0.04′** (Drik uses the same computational Lahiri we do).
- **Lagna vs Swiss Ephemeris**: matches to **≤ 1′** across both hemispheres and
  the full day; cross-checked **live against ProKerala** at the degree level
  (Leo 5°59′ = our value; Drik's Leo 6°10′ is the documented outlier).
- **Moon nakshatra** agrees with Swiss Ephemeris's Moon at every test epoch.
...
For perspective: **0.75′ of arc ≈ 3 seconds of birth time.** No one knows their
